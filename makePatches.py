...
def imgToPatch(name_list):
    train_filename = 'pairs_ostraca2.tfrecords'					
    writer = tf.io.TFRecordWriter(train_filename) # MODF
    ...
    d=0
    size = 800
    for it,filename in enumerate(os.listdir("./Data/Pictures")): # MODF
        if (".jpg" in filename and filename in name_list): # MODF
            image = cv2.imread("./Data/Pictures/"+filename) # MODF
            print(it+1)
            print(filename)
            ...
            for i in range(0,image.shape[0],size):
                ...
                # then write the pair in the TFRecord file
                    try:
                        ...
                        del rec
                        pm2 = zoom(np.transpose(pm2,(1,0,2)),(0.5,0.5,1))
                        ph = zoom(np.transpose(ph,(1,0,2)),(0.5,0.5,1))
                        c += 1
                        feature = {
                    ...
                    try:
                        ...
                        del rec
                        pm3 = zoom(np.transpose(pm3,(1,0,2)),(0.5,0.5,1))
                        pb = zoom(np.transpose(pb,(1,0,2)),(0.5,0.5,1))
                        c += 1
                        feature = {
                    ...
                    try:
                        ...
                        pg = zoom(pg,(0.5,0.5,1))
                        pm4 = zoom(pm4,(0.5,0.5,1))
                        del rec
                        c += 1
                        feature = {
                    ...
                    try:
                        ...
                        pm5 = zoom(pm5,(0.5,0.5,1))
                        pd = zoom(pd,(0.5,0.5,1))
                        del rec
